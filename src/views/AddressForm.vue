...
export default {
  created () {
    // routeのパラメータにidが含まれているかチェック
    if (!this.$route.params.address_id) return
    // getterメソッドの呼び出し、引数にアドレスのidを渡すことで該当する連絡先のオブジェクトが取得できる
  ...
  data () {
    return {
      //  空のオブジェクトを用意してtext-fieldの要素を格納していく
      address: {}
    }
  },
  methods: {
    // 保存ボタンが押されたときstoreにフォームに入力されたデータが格納され、連絡先一覧への遷移とフォームデータのリセットをしている
    submit () {
